...
%test_scn_path = fullfile(data_path,'data/test_jpg'); %CMU+MIT test scenes
%test_scn_path = fullfile(data_path,'extra_test_scenes'); %Bonus scenes
test_scn_path = fullfile('~/','Desktop/img');

label_path = fullfile(data_path,'data/ground_truth_bboxes.txt'); %the ground truth face locations in the test set
...
[np,dp] = size(features_pos);
[nn,dn] = size(features_neg);
Y = ones(np+nn,1);
